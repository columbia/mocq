    ...
    try {
        switch (name) {
            case "codeql_complete": {
                const completions = await codeqlServer.getCompletions(args.file_uri, args.line, args.character, args.trigger_character);
            ...
            case "codeql_hover": {
                const hover = await codeqlServer.getHover(args.file_uri, args.line, args.character);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_definition": {
                const definition = await codeqlServer.getDefinition(args.file_uri, args.line, args.character);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_diagnostics": {
                const diagnostics = await codeqlServer.getDiagnostics(args.file_uri);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_format": {
                const edits = await codeqlServer.formatDocument(args.file_uri, args.range);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_references": {
                const references = await codeqlServer.getReferences(args.file_uri, args.line, args.character);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_open_file": {
                await codeqlServer.openDocument(args.file_uri, args.content);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_update_file": {
                await codeqlServer.updateDocument(args.file_uri, args.content);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "codeql_compile": {
                const queryPath = args.query_path;
                ...
                if (searchPath)
                    cmd.push("--search-path", searchPath);
                try {
